...
class TestStore {
  ...
  async readByUid(uid) { return [...this.events.values()].find((event) => event.uid === uid) ?? null; }
...
test("canonical model preserves stable UID and increments revision", () => {
  const event = normalizeEvent({uid: "family-1@example.test", title: "Practice", start: {kind: "date", date: "2026-09-21"}, end: {kind: "date", date: "2026-09-22"}}, {id: "event-1", revision: 3, now: "2026-09-20T12:00:00.000Z"});
...
test("model rejects mixed all-day and timed boundaries", () => {
  assert.throws(() => normalizeEvent({title: "Broken", start: {kind: "date", date: "2026-09-21"}, end: {kind: "utc", dateTime: "2026-09-22T00:00:00Z"}}), /same boundary kind/);
});
...
test("service enforces revision checks, idempotency, cancellation, and tombstones", async () => {
  ...
  const created = await service.create({event: {uid: "uid-1", title: "Dinner", start: {kind: "utc", dateTime: "2026-09-21T22:00:00Z"}, end: {kind: "utc", dateTime: "2026-09-21T23:00:00Z"}}, policy: agentPolicy});
...
test("iCalendar rendering escapes, folds, preserves all-day and UTC values, and emits cancellations", () => {
  const events = [
    normalizeEvent({uid: "all-day@example.test", title: "Family, holiday; long title that must be folded across a standards-compliant content line because it exceeds seventy-five octets", start: {kind: "date", date: "2026-12-24"}, end: {kind: "date", date: "2026-12-26"}}, {id: "a", revision: 2}),
    normalizeEvent({uid: "cancelled@example.test", title: "Cancelled", status: "cancelled", start: {kind: "utc", dateTime: "2026-09-21T20:00:00Z"}, end: {kind: "utc", dateTime: "2026-09-21T21:00:00Z"}, recurrence: ["RRULE:FREQ=WEEKLY"]}, {id: "b", revision: 4}),
  ];
...
test("named time zones require a complete VTIMEZONE component", () => {
  const event = normalizeEvent({uid: "zoned@example.test", title: "School", start: {kind: "zoned", dateTime: "2026-09-22T08:00:00", timeZone: "America/New_York"}, end: {kind: "zoned", dateTime: "2026-09-22T09:00:00", timeZone: "America/New_York"}}, {id: "z"});
