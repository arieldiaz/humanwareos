...
export class CalendarService {
  ...
  async mutate(action, input, policy) {
    ...
    if (!current && input.uid) current = await this.store.readByUid(input.uid, input.recurrenceId);
    ...
    if (action === "create") {
      ...
      event = reviseEvent(current, action === "cancel" ? {status: "cancelled"} : input.event, timestamp);
    }
  ...
  async applyInvitation(proposal) {
    authorizeMutation(proposal.policy, {allowInbound: true, trustedRecipients: this.trustedInvitationRecipients});
    const current = await this.store.readByUid(proposal.event.uid, proposal.event.recurrenceId);
    ...
    if (proposal.action === "cancel") {
      ...
      return this.cancel({id: current.id, expectedRevision: current.revision, policy: proposal.policy});
    }
    if (current) return this.update({id: current.id, expectedRevision: current.revision, event: proposal.event, policy: proposal.policy});
    return this.create({event: proposal.event, policy: proposal.policy});
  }
}
