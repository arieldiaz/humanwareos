...
`patch-2026.9.1-recovery-capability-boundary.mjs` prevents restart recovery from redispatching an interrupted turn into its former model or harness. Confirmed terminal work remains a no-op. A durably captured final reply may be sent unchanged only when its idempotency key and exact originating destination are present and still match the session. Every other interrupted turn is tombstoned as `needs_human_reauthorization` and receives an origin-bound notice through the gateway delivery queue. This is the strict first implementation of `docs/session-recovery.md`: it intentionally provides fewer recovery capabilities instead of trusting a resumed Cursor, Codex, or embedded session with stale mutation authority.
