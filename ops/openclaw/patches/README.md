...
`patch-2026.9.1-slack-response-reliability.mjs` makes a terminal event from the session's current writer authoritative when restart bookkeeping still names an older recovery run, preventing a successful Slack turn from remaining falsely `running`. Session-start conflicts remain retryable until the ordinary age floor rather than being dead-lettered after eight quick attempts, and persisted or inspected ingress payloads redact credential-shaped fields. Run `scripts/repair-openclaw-terminal-sessions.mjs` after stopping the gateway to preview any existing rows with durable terminal evidence, then rerun it with `--apply` before activation. The repair is evidence-gated and does not close a row unless its current lifecycle writer's latest durable event is `session.ended`.
