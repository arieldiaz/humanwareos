...
3. Use time plus a short slug, not a permanent taxonomy. Current indexes belong under `derived/` or `memory/current/`.
...
Transcripts, summaries, indexes, and reports belong under `$HUMANWARE_DATA_ROOT/derived/`. Each carries source identifiers, creation time, model or tool, schema version, and privacy scope. They are replaceable.

Curated facts and decisions are appended under `memory/events/`, then merged into compact files under `memory/current/`. Strategy has the same split between evidence and current projection. Working documents live under `workspaces/<owner>/` with version history. Published or reviewable revisions live under `artifacts/` and are immutable once addressed.
