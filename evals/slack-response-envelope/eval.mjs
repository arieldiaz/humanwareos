...
import {decodeFinal} from "../../ops/openclaw/plugins/run-signature/final-envelope.mjs";
...
export function gradeEnvelope(text, expect) {
  const failures = [];
  let envelope;
  try { envelope = decodeFinal(text); } catch (error) { return {pass: false, failures: [error.message]}; }
  if (envelope.status !== expect.status) failures.push(`expected status ${expect.status}`);
  if (expect.mustMatch && !new RegExp(expect.mustMatch, 'i').test(envelope.message)) failures.push('required evidence is absent');
