...
test("grades observable response state rather than exact wording", () => {
  assert.equal(gradeEnvelope("A concise result.", {shape: "short", status: "act"}).pass, true);
  ...
  assert.equal(gradeEnvelope("## Session Closed", {shape: "short", status: "done"}).pass, true);
...
test("rejects the overlapping protocols this change retires", () => {
  for (const response of ["Goal: fix it", "## Status\nNo action needed.", "Agent — working: checking"] ) {
    assert.equal(gradeEnvelope(response, {shape: "short", status: "act"}).pass, false);
  }
});
