...
28. **Fix the foundation, never productize a workaround.** A local workaround may contain immediate damage, but it is never the recommended user path, the definition of done, or a substitute for the owning-layer fix. Reusable behavior belongs upstream, instance policy belongs in private configuration, accumulated material belongs in data, and generated runtime is rebuilt rather than patched. Prefer one smaller canonical path that deletes superseded branches, reduces state and lines of code, and makes the correct behavior the easiest behavior. When the foundation cannot yet support the request, name the missing capability and repair it; do not hand the complexity back to the human as commands, rituals, or permanent operator knowledge.
