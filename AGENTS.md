...
9. **Share repositories safely.** Assume other sessions are active. Re-read a file immediately before editing it. Multi-file or durable source changes use a task-owned branch and isolated worktree from the current remote tip. Agents may build, test, and open a pull request, but only the human may approve its merge or a production restart or cutover; approval for implementation or an earlier release does not carry forward. Before requesting merge approval, show the pull-request link, production and test/spec lines added and removed, files added and deleted, complexity removed, and any new permanent surface. Do not restructure or mass-edit without the human's explicit approval.
