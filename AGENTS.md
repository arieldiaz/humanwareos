...
21. **Absolute paths, always** — including the machine username — because the human moves between machines and threads. A link may supplement a path, never replace it. In chat these stay raw, not clickable; rule 12's markdown-link requirement is about links written into repo files.
