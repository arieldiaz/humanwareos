...
Feed ordering is deterministic. The feed revision and ETag are derived from the selected calendar identity and sorted event IDs, revisions, and tombstone/cancellation state, so unchanged content retains its validator. Feed routes support conditional GET and set a read-only calendar content type. When a calendar client cannot send authentication headers, an instance may issue a separate revocable capability URL per subscriber or device. Its path token must carry at least 256 bits of entropy, never appear in query parameters or access logs, and be protected from referrer leakage; ordinary account bearer tokens remain forbidden in feed URLs. Access, cache, and publication policy belong to the private instance.
