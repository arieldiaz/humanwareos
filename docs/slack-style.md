...
- **No emoji in headings.** The sole exception is a genuine closing `❓ Clarify` / `✋ Act` handoff, whose emoji is load-bearing — copy those two exactly. An ordinary reply ends without a lifecycle header; never add one for visual consistency.
...
Fixed rules, then channel overrides, then the human's in-thread instruction (which applies to the whole thread going forward), then shape guidance. Brevity never suppresses a genuine lifecycle handoff and shape never creates one.
